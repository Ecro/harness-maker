...
provenance: official
content_hash: 927ca24e2989a93e69b55917dd9a3df28b0e09f2c6f0ad3428b8099a19513fc1
---
# /hm:loop
...
```bash
!uv run --with /home/noel/harness-maker/.worktrees/execute-20260523T0815Z python -m harness_maker.worktree create execute "$(pwd)"
```
...
5. **Finalize worktree** (only if engaged in step 5):
   ...
   ```bash
   !uv run --with /home/noel/harness-maker/.worktrees/execute-20260523T0815Z python -m harness_maker.worktree finalize <WT> <STATUS>
   ```
...
`/hm:loop p5-batch-all`), the loop enters the **prompt-driven SPEC bulk
authoring path**. This is NOT routed through `autoloop_driver.run()` —
ADR-013 R2 explicitly requires the slash command to handle batching here.
...
3. **4-gate per SPEC** (run after refinement, before mark_complete):
   ...
   |------|------|----------------|
   | a. schema | `python -m harness_maker.spec_machine validate {yaml}` | exit 0 |
