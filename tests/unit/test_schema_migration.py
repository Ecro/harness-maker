...
def test_side_defaults_new_fields_schema_v4(tmp_path: Path) -> None:
    """New Side harness (schema v4) renders the 5-term inequality gate schema
...
def test_schema_version_field_present_in_models() -> None:
    ...
    agent_models + default_model rename, 2 → 3 per PLAN-second-opinion-multi-model
    ADR-001 for the codex_second_opinion → second_opinion rename, and 3 → 4 per
    PLAN-harness-diet ADR-002 for the retired fused-workflow axis; InterviewAnswers mirrors.
    """
    hc = HarnessConfig()
