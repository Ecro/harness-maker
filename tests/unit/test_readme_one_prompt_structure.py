...
def test_codex_branch_uses_native_install_and_bundled_bootstrap(readme: Path) -> None:
    """AC-001 supersedes the old unavailable-marketplace constraint.

    Native CLI installation is exercised by test_codex_claude_live; this guard
    ties README instructions to the actually bundled first-run skill.
    """
    ...
    joined = " ".join(bash_lines)
    assert "codex plugin marketplace add Ecro/harness-maker" in joined
    assert "codex plugin add harness-maker@harness-maker" in joined
    assert "codex plugin install" not in joined
    assert "bundled hm-make skill" in sub
    assert "matching Python engine" in sub
    assert (REPO_ROOT / "skills/hm-make/SKILL.md").is_file()
