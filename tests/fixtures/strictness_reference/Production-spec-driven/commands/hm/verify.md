...
description: Pre-completion stop sign — deterministic regression, structure and security
  checks.
content_hash: 12b8d74ea2a7bf015f26c73a99899c8461eea3926e840cec7df7cd46e92ffd7a
---
...
- `work-docs/PLAN-{slug}.md` and `specs/SPEC-{slug}.md` (when present) — drive Check 6 and inform the `--force` rationale.
...
manual-checklist exit criterion has **no** automated re-check at this stage, and Check 6 gates the SPEC operation, not scenario coverage.
**`--no-tdd` path:** `/hm:execute --no-tdd` skips Phase A/A.5, and this stage does not
re-derive scenario coverage. The mechanical backstop is `/hm:wrapup` Step 3.5's
`spec_machine find-unbound` gate (fail-closed, STOP on this preset): a closed-type AC still `pending_test`
after the write-back cannot be committed, whichever way execute ran.
...
```bash
!uv run --with $HOME/harness-maker hm spec_need op-check --verdict <spec_need_verdict> --target <spec_need_target> --root <WT> --changed-file <f1> --changed-file <f2> …
```
...
```bash
!uv run --with $HOME/harness-maker hm spec_need waiver-check --root <WT> --slug <task_slug> --changed-file <f1> --changed-file <f2> …
```
...
To resolve: author the SPEC via /hm:spec <slug> (the
resume-marker skips re-detection and routes directly to /hm:spec) or record
a fresh, non-empty-rationale waiver with:
  uv run --with $HOME/harness-maker hm spec_need waiver-set --root <WT> --slug <slug> \
...
[1/6] Drift verdict (REVIEW present) ✅ PASS
...
```bash
!uv run --with $HOME/harness-maker hm worktree task-preflight <slug> "$(pwd)" --stage hm:verify --claude-session-id "$HM_SESSION_ID"
```
