...
content_hash: 5c0da9ef6feabda945114e29d1067a2874cccecc3bf5c4a5829ac4236fc8e0b5
---
...
```bash
!uv run --with $HOME/harness-maker hm worktree task-preflight <slug> "$(pwd)" --stage hm:review --claude-session-id "$HM_SESSION_ID"
```
...
```bash
!cd <WT> && uv run --with $HOME/harness-maker hm review_run open --slug <slug>
```
...
**The discovery axis** — `design`, `functionality`, `robustness`, `consistency`, `security`, `concurrency`, `tests` —
...
- `routing: conditional` → use Conditional Router (M6) on the changed-file paths. It may drop **only** `ux-reviewer` / `performance-reviewer`; a mandatory lens is never routed away, because incomplete coverage blocks approval and a routed-away lens would make every conditional review permanently unapprovable.
- Add any extras from `--with-reviewers=<csv>`.
- Resolve `review_base` once, here in round 1, storing it at `refs/hm-freeze/v1/<slug>-base`:

  ```bash
  !cd <WT> && uv run --with $HOME/harness-maker hm freeze resolve-base --slug <slug>
  ```
...
```bash
!cd <WT> && uv run --with $HOME/harness-maker hm lens_coverage check --results-dir .claude/observability/.hm-lens-results --slug <slug> --round <round> --run-id <run-id> --preset Production
```
...
1. Build `pass1_context` from the diff context with PR title / description /
   author / commit message redacted. `Write` the JSON context to a temp path, then run the
   harness CLI on that path rather than redacting in prose — **never `echo '<json>' |`**:
   the context carries the diff verbatim, and one apostrophe in a changed line ends the
   quoting and hands the rest of that line to the shell.
   ```bash
   uv run --with $HOME/harness-maker hm two_pass_review redact --file <the literal temp path>
   ```
   The CLI returns a JSON object with the same fields but anchoring values
   replaced by `[REDACTED]`.
2. Run all selected reviewers in a **single message, one sub-agent dispatch per
   reviewer** for parallel execution, passing `pass1_context`. Each reviewer:
...
> **No verifier step runs between the passes (ADR-001).** The Pass 1.5 `code-verifier`
> dispatch was removed. It dropped 5 of 261 findings (1.9%) across 41 reviews, and — the
> operative reason — it was a full serialized agent round-trip on the critical path of
> every review round while nothing else ran. **Pass 2 consumes the raw Pass 1 findings
> list.** The `code-verifier` agent itself is unchanged; do not re-add a mode A dispatch here.
...
```bash
!uv run --with $HOME/harness-maker hm world objective show <id> --json
```
...
```bash
!cd <WT> && uv run --with $HOME/harness-maker hm codex_adapter stamp-ids < <the literal temp path>
```
...
```bash
!cd <WT> && set -o pipefail; uv run --with $HOME/harness-maker hm review_consensus finalize --file <the literal temp path> --slug {slug} --round <N> --spec specs/SPEC-{slug}.machine.yaml | tee <FINALIZE-r{N}>
```
...
> **Drop the `--spec` argument if `specs/SPEC-{slug}.machine.yaml` does not exist**
> — a slug whose SPEC stage recorded `none` has no machine SPEC, and the grade is then computed
> with no AC citation able to clear it, which is the intended fail-closed outcome. The CLI already
> degrades that way on its own; omitting the flag just keeps a routine run from reporting an error
> every round, which is what would teach a reader to ignore the exit code the other checks rely on.
...
```bash
!cd <WT> && uv run --with $HOME/harness-maker hm lens_coverage check --results-dir .claude/observability/.hm-lens-results --slug <slug> --round 1 --round 2 <one --round per further round so far> --round <this round> --run-id <run-id> --preset Production
```
...
```bash
!cd <WT> && uv run --with $HOME/harness-maker hm review_churn pin --slug {slug} --label r{N}-pre
```
...
```bash
!cd <WT> && set -o pipefail; uv run --with $HOME/harness-maker hm review_churn pin --slug {slug} --label r{N}-post && uv run --with $HOME/harness-maker hm review_churn measure --slug {slug} --round {N} --pre refs/hm-churn/v1/{slug}-r{N}-pre --post refs/hm-churn/v1/{slug}-r{N}-post | tee <CHURN-r{N}>
```


5c. **Measure what the round did to the code**, not only how much it edited. Churn counts edits;
   this counts what the edits left behind. Its own call on purpose — a telemetry failure must
   never take 5b's churn measurement down with it, and this feature records rather than gates.

```bash
!cd <WT> && uv run --with $HOME/harness-maker hm review_churn complexity --slug {slug} --round {N} --pre refs/hm-churn/v1/{slug}-r{N}-pre --post refs/hm-churn/v1/{slug}-r{N}-post
```

   It reuses 5b's endpoints, so it measures the round AFTER reverts — a reverted fix leaves no
   complexity behind either. `complexity_status` is three-valued: `not-python` is ADR-001's
   accepted cost made visible, `unparseable` is a transient state, and a `measured` file always
   scores at least 1 — so a null never has to be guessed at.
...
6. **Re-review (gated on Step 5b's ratio).** Three branches, and the third is the one that matters:
   ...
   - **`churn_ratio` < `0.3`** → **dispatch nobody.**
...
```bash
!cd <WT> && uv run --with $HOME/harness-maker hm review_consensus plan --churn-ratio <the measured ratio> --threshold 0.3
```
...
```bash
!cd <WT> && uv run --with $HOME/harness-maker hm freeze commit --slug <slug> --pass <confirm-1|confirm-2>
```
...
```bash
!cd <WT> && uv run --with $HOME/harness-maker hm freeze read-base --slug <slug>
```
...
```bash
!cd <WT> && uv run --with $HOME/harness-maker hm lens_coverage check --results-dir .claude/observability/.hm-lens-results --slug <slug> --round <pass-id> --run-id <run-id> --preset Production
```
...
```bash
!cd <WT> && uv run --with $HOME/harness-maker hm freeze reap --slug <slug>
```
...
```bash
!cd <WT> && uv run --with $HOME/harness-maker hm review_churn oscillation --slug {slug} --rounds <rounds>
```
...
|------|-----|------------|-------------|--------|
| {path} | {pre_loc} → {post_loc} | {pre} → {post} | {pre} → {post} | {complexity_status} |

Read it from 5c's rows — do not re-derive the numbers. **Report only; there is no threshold and
no gate.** A study measuring ten arms over 47 rounds found the compliance verdict identical
(19/19) while complexity split −17% against +58%, which is the case for recording it; it supplies
no threshold, and inventing one here would be a device that costs more than it protects.

Write `null` as `null`, never `0`: a non-Python file and a perfectly simple one are different
facts, and `complexity_status` says which you are looking at.
...
Round-level numeric fields default to 0, **except `wall_time_ms`, which is optional and
defaults to `null`** — the warning below forbids interpolating it, so requiring it made the
two instructions contradict. Send an integer when measured, omit it otherwise — never `0` for an
unmeasured round.
`fixture_label` / `verifier_false_*` / `fallback` are null on real runs. **The `verifier_kept_n` / `verifier_dropped_n`
...
```bash
!uv run --with $HOME/harness-maker hm review_telemetry emit --file <the literal temp path> --measured <FINALIZE-r{N}> <plus --measured <CHURN-r{N}> from round 2>
```
