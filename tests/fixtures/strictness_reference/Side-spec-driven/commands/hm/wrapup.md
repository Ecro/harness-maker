...
description: 'Close the unit of work: final verification, memory capture, and the
  single commit.'
content_hash: 7dd5a4d2385da236dec4df55ae655ea8868a7d5e6f72503af40f6c0528634797
---
...
commit, `post-commit-pop`, `owned-crumb-clear`, `drain` — and returns a JSON receipt.**Before landing — the DRI's acceptance of the SPEC.** Read the task SPEC's state from the
checkout that is about to land:


```bash
!uv run --with $HOME/harness-maker hm spec_machine approval-status --root "$(pwd)" --slug <slug> --checkout "$(pwd)"
```
...
- `land: hold` → show the DRI each `irreversible` id with its decision from the SPEC (and
  ...
  `hm spec_machine approve --yaml "$(pwd)"/specs/SPEC-<slug>.machine.yaml`,
