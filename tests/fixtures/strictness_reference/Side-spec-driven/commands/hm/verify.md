...
description: Pre-completion stop sign — deterministic regression, structure and security
  checks.
content_hash: 0c884261cdbb905129901a242f934fc480d3e0da66db9b7c8baff439a8f462aa
---
...
re-derive scenario coverage. `/hm:wrapup` Step 3.5's
`spec_machine find-unbound` scan runs on this preset too but is **advisory only** (Side never STOPs),
so an AC still `pending_test` after `--no-tdd` is surfaced in the wrapup report, not blocked.
...
```bash
!uv run --with $HOME/harness-maker hm spec_need op-check --verdict <spec_need_verdict> --target <spec_need_target> --root . --changed-file <f1> --changed-file <f2> …
```
...
```bash
!uv run --with $HOME/harness-maker hm spec_need waiver-check --root . --slug <task_slug> --changed-file <f1> --changed-file <f2> …
```
...
a fresh, non-empty-rationale waiver with:
  uv run --with $HOME/harness-maker hm spec_need waiver-set --root . --slug <slug> \
