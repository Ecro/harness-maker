...
content_hash: d0d457755eb68e07c340357d77cca4bbdde37c6073b05b2f721ddc16b68b99d9
---
...
Write to `./work-docs/PLAN-{slug}.md`. The path is
...
```bash
!uv run --with $HOME/harness-maker hm spec_need record --verdict '<verdict>' --target '<target>' --root . --rationale '<why>'
```
...
```bash
!uv run --with $HOME/harness-maker hm spec_need frontmatter-upsert --plan 'work-docs/PLAN-<slug>.md' --root . --verdict '<verdict>' --target '<target>'
```
...
`.claude/.hm-iter-receipts/.current-iter`, find the next phase whose status is not
