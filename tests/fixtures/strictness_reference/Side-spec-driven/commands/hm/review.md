...
content_hash: 957690749f8b0583ddc0b880b86123de823c0620dce85d2fc5bdb56854237b70
---
...
```bash
!uv run --with $HOME/harness-maker hm review_run open --slug <slug>
```
...
```bash
!set -o pipefail; uv run --with $HOME/harness-maker hm review_consensus finalize --file <the literal temp path> --slug {slug} --round <N> --spec specs/SPEC-{slug}.machine.yaml | tee <FINALIZE-r{N}>
```
...
```bash
!set -o pipefail; uv run --with $HOME/harness-maker hm review_churn pin --slug {slug} --label r{N}-post && uv run --with $HOME/harness-maker hm review_churn measure --slug {slug} --round {N} --pre refs/hm-churn/v1/{slug}-r{N}-pre --post refs/hm-churn/v1/{slug}-r{N}-post | tee <CHURN-r{N}>
```
...
```bash
!uv run --with $HOME/harness-maker hm review_consensus plan --churn-ratio <the measured ratio> --threshold 0.3
```
...
> a bare filename. A later step may run from a different directory than the producers did;
