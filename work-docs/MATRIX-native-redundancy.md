...
## Appendix — step sensitivity class per rendered step (PLAN-workflow-steps-vs-model-capability)

Generated by `harness_maker.step_sensitivity.matrix_rows(REGISTRY)`; the structural test
`test_docs_carry_classes` asserts every row below is present, so edit the registry, not this table.
Class: **COMP** shrinks as models improve · **HOST** the host harness now does it · **INV** keep
regardless · **TUNE** value flips per model, re-measure before transferring. Grade: `***` reproduced
across models, `**` measured once, `*` judgement, `unsourced` inherited with no RESEARCH row
(39 of 82). TUNE evidence is **Side-preset only** (stage-agent ledger has no
Production rows).

| stage | ordinal | class | grade |
|---|---|---|---|
| research | Phase 0 | INV | unsourced |
| research | Phase 0.75 | COMP | unsourced |
| research | Phase 1 | INV | ** |
| research | Phase 2 | INV | ** |
| research | Phase 3 | INV | ** |
| research | Phase 4 | INV | unsourced |
| spec | Step 0 | INV | unsourced |
| spec | Step 1 | INV | unsourced |
| spec | Step 2 | INV | *** |
| spec | Step 3 | INV | unsourced |
...
| plan | Step 2 | INV | unsourced |
| plan | Step 3 | INV | ** |
| plan | Step 3.0 | INV | unsourced |
| plan | Step 4 | TUNE | ** |
...
| plan | Step B | INV | unsourced |
| plan | Step C | INV | unsourced |
| plan | Step D | INV | unsourced |
| plan | Step E | COMP | * |
...
| execute | Step 3 | INV | *** |
| execute | Phase A | INV | *** |
| execute | Phase A.4 | INV | ** |
| execute | Phase A.5 | TUNE | ** |
| execute | Phase B | INV | *** |
| execute | Phase C.0 | COMP | * |
| execute | Phase C | INV | unsourced |
| execute | Phase D | INV | unsourced |
| execute | Phase D.5 | COMP | * |
| execute | Step 4 | INV | unsourced |
| execute | Step 4.5 | INV | unsourced |
| execute | Step 5 | HOST | ** |
| review | Step 0 | INV | unsourced |
| review | Step 1 | INV | unsourced |
| review | Step 2 | INV | ** |
...
| verify | Check 6 | INV | ** |
| verify | Step 0.5 | INV | ** |
| wrapup | Step 1 | INV | unsourced |
| wrapup | Step 2 | INV | unsourced |
| wrapup | Step 3 | INV | unsourced |
| wrapup | Step 3.5 | INV | unsourced |
| wrapup | Step 3.6 | INV | unsourced |
| wrapup | Step 4 | INV | unsourced |
| wrapup | Step 5 | INV | ** |
| wrapup | Step 7.7 | HOST | ** |
| wrapup | Step 8 | INV | unsourced |
| wrapup | Step 0.5 | INV | ** |
