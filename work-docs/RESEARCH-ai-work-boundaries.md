...
| **G8** | **Research stage makes no decided/undecided split and never reads what existing tests pin.** The 7 required sections (`research.md.j2`) have no place for "already fixed by the current contract / current tests" vs "open". The synthesis's second research rule — check whether an existing test pins a **production-unreachable** state — has no counterpart anywhere in `research.md.j2` (`reachab` matches only in `execute.md.j2` D.5 and `wrapup.md.j2`). | [데이터] all 4/4 reproduced "regressions" were tests pinning unreachable states | `research.md.j2` Phase 1 + required sections | medium |
