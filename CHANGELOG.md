...
- **`tests/e2e/test_preservation_e2e.py`** — 5 INTEGRATION=1-gated scenarios verifying end-to-end on-disk preservation: Claude PascalCase hooks.json merge, Cursor flat-schema hooks.json merge, Codex PermissionRequest (matcher-less, nested) merge, `.codex/config.toml` user-block survival (currently **xfail-strict** — see "Phase 2 render-merge half-shipped" note below), `.backup-*/` auto-gitignore wiring + idempotency. 4 GREEN + 1 xfail.
