...
- **Codex dual role** (PLAN-codex-second-llm-integration ADR-009): `codex` 는 IDE asset 렌더링 (`.codex/`) 뿐 아니라 second-LLM provider 역할도 한다. `harness.yaml.codex_second_opinion.enabled=true` 면 `code-reviewer / consensus-arbiter / plan-validator` 가 `codex exec` 을 호출 (Bash dispatch only, no MCP). 두 축은 직교 — `targets` 에 `codex` 없어도 `codex_second_opinion` 활성화 가능 (사용자 측 `codex` CLI + `codex login` 만 있으면 됨). 실패 정책은 warn-and-proceed.
  - **Cross-model deepening** (PLAN-crossmodel-codex-gaps, enabled=true 시): (1) `/hm:review` 가 Codex 를 **k-of-3 consensus 정식 투표권**으로 합류시킨다 (Step 3.5 orchestration → `codex_adapter` 로 severity `critical→P0…` 매핑 + null-location symbol/message-similarity relaxation → Step 4 filter). (2) **mandatory 매트릭스**: Production = review+plan 항상 강제 / Side = high-diff 시에만 (`harness_maker.high_diff` 판정, loop 에선 per-iter diff). (3) plan-validator 는 **PIDA** (KEEP/REFUTE → oracle 없으면 `unresolved` surface, never-block). (4) skip/disposition 은 `.claude/observability/codex-second-opinion.jsonl` ledger 에 best-effort 기록 + `/hm:health` 가 positive smoke-test 로 silent-degradation 을 잡는다. mandatory 는 hard-block 아님 (codex login 미설정 시 loud-warn + skip-receipt). loop 는 stage-level 이라 자동 상속.
