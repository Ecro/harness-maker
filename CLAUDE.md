...
- **Codex dual role** (PLAN-codex-second-llm-integration ADR-009): `codex` 는 IDE asset 렌더링 (`.codex/`) 뿐 아니라 second-LLM provider 역할도 한다. 이 provider 축은 **`harness.yaml.second_opinion`** (PLAN-second-opinion-multi-model 이 옛 `codex_second_opinion` 을 대체) 로 제어된다 — `targets` 와 직교. 자세한 건 아래 **Cross-model second opinion (multi-model)**.
  - **Cross-model second opinion (multi-model)** (PLAN-second-opinion-multi-model, supersedes PLAN-codex-second-llm-integration + PLAN-crossmodel-codex-gaps): `harness.yaml.second_opinion.models: list[Literal["codex","antigravity"]]` — 활성 모델 집합 (빈 리스트=off). Codex CLI (`codex exec`) 와 Antigravity CLI (`agy --print --sandbox`, project-less) 를 각각 독립 second-opinion voter 로 붙일 수 있고 **둘 다 동시** 가능. 옛 `codex_second_opinion.enabled=true` harness 는 `answers_from_harness_yaml` 의 1회성 silent migration (ADR-001, schema_version 2→3) 으로 `second_opinion.models=["codex"]` 로 자동 변환 (both-keys-present 면 new-key-wins + advisory 1회). 축은 `targets` 와 직교 — `targets` 에 `codex`/`agy` 없어도 활성 가능 (사용자 측 CLI + login 만 있으면 됨).
    ...
    - **graceful degrade** (ADR-011): CLI 미설치/삭제(exit 127) / login 만료 / rate-limit·구독 초과 / timeout(agy 는 hang 가능 → agy native `--print-timeout 240s` 필수 — external `timeout` 래퍼 금지, print-timeout 초과 시 non-zero exit → graceful skip) / 파싱 불가 — 전부 warn-and-proceed skip. Codex 는 `--output-schema` 로 JSON 강제; **antigravity 는 CLI-레벨 강제 없음** → `codex_adapter.extract_antigravity_payload` 가 fail-closed (fence strip + 정확히 1개 JSON payload 아니면 `status: failed`, never crash). plan-validator 는 **PIDA** (KEEP/REFUTE → oracle 없으면 `unresolved` surface, never-block).
    ...
    - skip/disposition 은 `.claude/observability/second-opinion.jsonl` (모델별 `model` 필드) ledger 에 best-effort 기록 + `/hm:health` 가 per-model positive smoke-test 로 silent-degradation 을 잡는다. antigravity sandbox 안전성은 Phase-1 write-probe 로 empirically 검증됨 (`tests/manual/ANTIGRAVITY_SANDBOX_PROBE.md`, ADR-012): project-less `agy --print --sandbox` 는 file 도구 미노출 → working-tree mutation 불가. loop 는 stage-level 이라 자동 상속.
