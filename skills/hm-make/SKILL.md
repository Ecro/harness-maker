...
Use the requested project directory. Reuse existing configuration. For a new project, collect any missing meaningful preferences (Side or Production preset, locale); respect preferences already given. The script requires Python 3 and uv on PATH.

Run `python3 "<installed-plugin-root>/scripts/codex_setup.py" make "<project>"` with explicit `--preset` and `--locale` when chosen (SPEC strictness derives from the preset; change it later with `/hm:configure`). The bundled bootstrap selects the published engine version matching the plugin release and generates Codex assets through the engine's normal generator.
