...
- **A seven-lens review axis, where one lens is enough to carry a finding.** Round 1 dispatches `design`, `functionality`, `robustness`, `consistency`, `security`, `concurrency` and `tests` — six textbook categories merged down from nine on measured redundancy (`consistency` duplicated another lens on 80 % of its findings; `security`, `concurrency` and `tests` on 0 %). Production makes all seven mandatory; Side makes the four core ones mandatory and routes the three domain lenses — but **both dispatch all seven**, because a router can only drop what was dispatched. A finding raised by exactly one lens is `consensus-passed`: two lenses agreeing is corroboration, and requiring it discards whatever only one specialist could have seen. K=2 still applies to cross-model voters and to the same lens speaking twice.
