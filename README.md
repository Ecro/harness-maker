...
Install path per IDE:
  ...
  Codex CLI:    Bash  codex plugin marketplace add Ecro/harness-maker
                Bash  codex plugin add harness-maker@harness-maker
                Load the bundled hm-make skill; it resolves the installed
                package and runs its matching Python engine through uv.
                If skills are not discovered, start a new Codex thread.
                Use hm-update later to update plugin, engine, and project.

After install, run hm-make on Codex or harness-maker:make elsewhere (drive the interview — accept defaults
unless I object), then hm:health (read out the personalization tier and top
action items).
```

> **Bash approval:** approve only the exact install commands shown above (`codex plugin marketplace add …`, `codex plugin add …`, `claude plugin install …`, `git clone --depth 1 https://github.com/Ecro/harness-maker.git …`, `uv tool install harness-maker`). Do **NOT** grant blanket `Bash(*)`. If the AI requests a different command, stop and inspect.
...
| 🛡️ | **Trusted** | With `worktree.enabled: true` (the Production default) every `/hm:` stage for a task runs in one **persistent** worktree — `.worktrees/<slug>/` on branch `hm/<slug>`, **shared** by plan/execute/review and squash-landed by `/hm:wrapup` — and the TDD loop runs there. With `worktree.enabled: false` (the Side default) **no worktree** is created and every stage runs in the current tree. `/hm:review` doesn't just report — it applies consensus-passed fixes and re-reviews until grade ≥ A. Mechanical checks (lint/tests) gate the LLM reviewer before any token is spent. |
...
| **Second opinion** | `second_opinion.models` | A list of cross-model reviewer CLIs — `codex` (Codex), `antigravity` (`agy`), and `claude` (Claude Code), independently or together. Each enabled model casts a real k-of-N consensus vote in `/hm:review` — but only after its findings survive a refutation gate (`code-verifier` mode B) that dispositions each one against injected test-oracle output; only `accepted` findings vote. Models are invoked **once per review**, not per auto-fix round. Reconciled in `/hm:spec`; a missing/unauthenticated/rate-limited CLI degrades gracefully (warn + skip, never blocks). Requires the corresponding CLI (`codex login` / an authenticated `agy` / `claude` login); orthogonal to `targets` |
...
Codex invokes generated stage skills with `$hm-research topic`, `$hm-spec task-slug`, and `$hm-execute task-slug`. Claude Code and Cursor use `/hm:research`, `/hm:spec`, and `/hm:execute`. Research hands off to spec; execute writes the PLAN before implementation. The separate plan stage has been removed. Regenerate an existing project harness after updating the plugin to refresh its guidance.
...
| **Codex CLI** | 1 | 0 | 0–1 (new thread if plugin skills need discovery) | **2** |

> **Codex CLI install path.** Native installation exposes bundled `hm-make` and `hm-update` skills. These run the matching Python engine through `uv`; no Claude Code cache is required. Plugin installation alone does not generate the project harness.

> **First-use Bash approval.** Claude Code / Cursor / Codex CLI will request Bash permission for the install commands shown below. Approve **only** the exact commands in the prompt (`codex plugin marketplace add Ecro/harness-maker`, `codex plugin add harness-maker@harness-maker`, `claude plugin install harness-maker@harness-maker`, `git clone --depth 1 https://github.com/Ecro/harness-maker.git ~/.cursor/plugins/local/harness-maker`, or `uv tool install harness-maker`). Do **NOT** grant blanket `Bash(*)` — if the AI requests a different command, stop and inspect. The Cursor clone fetches the current `main` branch of a public repo; future `git pull` updates are not integrity-verified — pin a release tag manually if this matters for your threat model.
...
harness-maker:make skill is already available):
  ...
  IF Codex CLI:
    ...
    Bash: codex plugin add harness-maker@harness-maker
    Load the bundled hm-make skill. It runs the matching Python engine
    through uv and generates project-native Codex agents and stage skills.
    If the new plugin skills are not discovered, start a new Codex thread.

  IF you can't tell which IDE (or none of the above), STOP and ask me which
  IDE you're in.

Step 3 — Once plugin skills are available, invoke hm-make on Codex or
harness-maker:make elsewhere and drive the interview:
...
#### Codex CLI (native plugin setup and update)
...
Load `hm-make` from the installed plugin to generate the project harness. It uses Python 3 and `uv`, pins the engine to the plugin's stable release, and renders `.codex/`, `AGENTS.md`, and `.agents/skills/`. Start a new Codex thread if new skills are not yet discovered.

Use `hm-update` to refresh the Git marketplace, reinstall the plugin, and regenerate the project with the matching engine. The receipt reports plugin, engine, and project versions independently: `complete` requires all three to match. Existing targets and custom blocks are preserved. A local-path marketplace has no Git upgrade operation; register a Git source for this update path.

The engine must be published at the plugin's release version. Development cachebusters do not select a different engine. For engine-only setup, `uv tool install harness-maker` followed by `harness-maker make --targets codex` remains available.

For Claude second opinions, enable `second_opinion.models: [claude]` in `.claude/harness.yaml`. Optional `second_opinion.claude` keys are `model` (default: CLI selection) and `timeout` (positive seconds, default 300). The authenticated `claude` CLI must support `--safe-mode`; the invoker sends supplied context on stdin with tools disabled. Unsupported/authentication failures are recorded skips; malformed results are failures, never clean votes.
...
This path is fine for one-off renders but skips the in-IDE slash-command surface — `/hm:health`, `/hm:spec`, `/hm:execute`, etc. only appear after the IDE plugin loads them.
...
- **A seven-lens review axis, covered in four dispatches, where one lens is enough to carry a finding.** Round 1 covers `design`, `functionality`, `robustness`, `consistency`, `security`, `concurrency` and `tests` — six textbook categories merged down from nine on measured redundancy (`consistency` duplicated another lens on 80 % of its findings; `security`, `concurrency` and `tests` on 0 %). The four core lenses share one `code-reviewer` dispatch (they differed only by one brief sentence), leaving as a single `core.json` whose findings each carry their own member lens; the three domain lenses stay their own dispatches. Production makes all seven lenses mandatory; Side makes the four core ones mandatory and routes the three domain lenses — but **both cover all seven**, because a router can only drop what was dispatched. A finding raised by exactly one lens is `consensus-passed`: two lenses agreeing is corroboration, and requiring it discards whatever only one specialist could have seen. K=2 still applies to cross-model voters and to the same lens speaking twice.
- **Grade-based auto-fix loop, with three ways to stop.** `/hm:review` applies consensus-passed fixes → re-reviews → regrades. It ends when the grade meets the threshold **and a confirmation pass over a frozen artifact finds nothing new**, or when no lifecycle transition happened in a round (`no-progress`), or when `max_review_rounds` runs out — three different endings that are reported as three different things. Weak-consensus and manual-only findings are never auto-applied.
- **The repair round is gated on how much it rewrote.** Fixes introduce defects at close to 1:1, so each round's churn is measured over pinned pre/post trees — the maximum across touched files, so a one-line edit to a 5000-line file can't mask a small file rewritten whole. Below `reviewers.rereview_churn_ratio` the re-review is skipped and the comparison recorded; at or above it, exactly one structured reviewer runs. A round whose churn could not be measured re-reviews anyway. `rereview_churn_gate: false` restores the previous behaviour verbatim.
- **A hunk one round removes and a later round restores is a SPEC gap, not a defect.** It is reported as `manual-only` P1 `spec_gap` and never moves the grade — a review whose only real problem is that nobody wrote down which behaviour was wanted must still be able to reach A.
- **Test runs are sized to the machine and the runner.** `hm test_runners plan` names the project's runner, the three levers it actually has (parallel / change-based selection / re-run-failed), and a worker count capped near half the visible cores. `-n auto` is not portable advice: `cargo`, `go`, `vitest`, `jest` and `flutter` are already parallel, where a worker flag caps or nests rather than accelerates, and `pytest` is the one common runner that is serial by default. On Python projects the `targeted-test-selection` skill additionally selects by import graph, falling back to `mode: full` whenever the changed set can't be resolved — the safety property is the fallback, not the optimisation.
- **The loop re-derives before it batches.** Patching one finding at a time reproduces defects at roughly a 1:1 rate — on the task this came from, 9 of 30 findings were defects introduced by a previous round's own fix. Before editing, a round now re-derives the underlying state model when **either** ≥2 of its findings share a subsystem **or** any finding new that round is attributed to a prior fix (`caused_by`), and makes one consolidated edit per group. A lone, unattributed finding keeps the fast path, so the common case gets no slower. The normative contract lives in one place — the `second-opinion-gate` skill's round-state section — and the stage loads it rather than restating it.
- **Termination is reported, not assumed.** The REVIEW summary carries `unreviewed_fix_count` (fixes applied in the terminal round that the loop never re-reviewed), `regression_attributed_n` and `attribution_unknown_n`. They gate nothing; they exist so an `A` grade is not read as "settled". All are typed `| None` in the telemetry record, so "never measured" stays distinguishable from "measured zero".
...
- **Worktree isolation per task.** With `worktree.enabled: true`, one **persistent** worktree per task — `.worktrees/<slug>/` on `hm/<slug>`, **shared** by every `/hm:` stage and squash-landed once by `/hm:wrapup`, so two concurrent sessions cannot commit into each other's work. With `worktree.enabled: false` **no worktree** is created. Failed runs preserve evidence; cleanup is prefix-matched (Cursor's own worktrees never touched). Create also prunes stale harness-owned markers/worktrees first, and stale finalize-stash refs do not count as live queue pressure.
...
- **Arming works in every runtime; auto-advance supports Claude Code and Codex.**
  Claude dispatches through its Skill tool. Codex reads and executes the next local
  stage skill in the same conversation only after the shared boundary returns
  `proceed: true`. Missing skills produce an explicit handoff; Cursor retains
  manual stage starts. Approval, review, verification and cap gates still apply.
  Sessions without a usable session ID share the degraded marker.
- **Feedback follows ordinary work.** Entry/resume, material observations and
  closeout load the intent feedback procedure. Existing authorization is reused
  for the same scope; writes are read back before dependent decisions. SPEC and
  PLAN links lead to the existing intent and shared-state records.
  A separately activated trial observes the next three actual task slugs in
  first-start order. Implementation completion does not claim trial success.
...
- `.cursor/hooks.json` — Cursor-native hooks schema (lowercase camelCase keys, `version: 1`, flat `{matcher, command}`). **Deliberately different from where Claude Code reads hooks**, which is the `hooks` key inside `.claude/settings.json` (PascalCase, nested `{hooks:[…], matcher}`) — *not* `.claude/hooks/hooks.json`, which is a plugin-bundle path this project no longer renders. Each IDE reads only its own file; don't try to collapse them — Cursor will silently stop firing hooks. See `tests/cursor-compat/results-2026-05-08.md` for the kairos 0.5.7 forensic that proved the Cursor half empirically, and `docs/ARCHITECTURE.md` for why the Claude half of that same forensic was an untested assumption for two years.
