...
Install path per IDE:
  ...
  Codex CLI:    Install the Python engine via Bash (required — the engine
                renders the harness; the plugin assets alone do not):
                ...
                (Optional: recent Codex CLI also registers the plugin natively
                via `codex plugin add harness-maker@harness-maker`, but that
                only clones the repo — it does NOT install the engine, so still
                run A or B.)
                After install, the Skill tool not yet populated with
                harness-maker:make on Codex first-run — `.agents/skills/`
                are generated BY the make step itself. Run the FIRST make
                via Bash directly:
                     Bash  harness-maker make
                Subsequent invocations (and hm:health) become available
                to the Skill tool after this initial Bash-run populates
                `.agents/skills/`.
...
> **Bash approval:** approve only the exact install commands shown above (`claude plugin install …`, `git clone --depth 1 https://github.com/Ecro/harness-maker.git …`, `uv tool install harness-maker`). Do **NOT** grant blanket `Bash(*)`. If the AI requests a different command, stop and inspect.
...
> **Codex CLI install path.** Recent Codex CLI (verified on 0.144.4) *does* install this plugin via `codex plugin marketplace add` + `codex plugin add harness-maker@harness-maker` — but that only clones the repo into the plugin cache; it does **not** install the Python engine that renders the harness. So the reliable path is still to install the engine via Claude Code's marketplace (if `claude` CLI is on PATH) OR via PyPI (`uv tool install harness-maker`) — both produce the same `.codex/` artifacts on disk. A native `codex plugin add` is optional on top of that, not a replacement for the engine install.
...
harness-maker:make skill is already available):
  ...
  IF Codex CLI:
    Install the Python engine (required — the plugin assets alone
    don't render the harness). Recent Codex CLI also registers the
    plugin natively via `codex plugin add harness-maker@harness-maker`,
    but that only clones the repo and does NOT install the engine, so
    still run A or B:
    ...
    B. Otherwise install via PyPI:
       Bash: uv tool install harness-maker
    After install, the Skill tool not yet populated with
    harness-maker:make on Codex first-run — `.agents/skills/` are
    generated BY the make step itself. Run the FIRST make via Bash:
       Bash: harness-maker make
    Subsequent invocations (and hm:health) become Skill-tool-available
    once `.agents/skills/` exists.
...
#### Codex CLI (native plugin install works, but the engine still comes from Claude Code or PyPI)

**Recent Codex CLI (verified on 0.144.4) installs this plugin natively** — `codex plugin marketplace add Ecro/harness-maker` followed by `codex plugin add harness-maker@harness-maker` succeeds, cloning the repo into `~/.codex/plugins/cache/`. **But that clone does not install the Python engine** (`harness-maker` / `python -m harness_maker`) that actually renders the harness, and it drags in the entire working tree (`.git`, caches, backups) rather than a clean release. So don't rely on native `plugin add` alone — install the engine via one of the two paths below; a native `codex plugin add` is optional on top, not a substitute. The `.codex-plugin/plugin.json` manifest carries metadata only (no install hook).
