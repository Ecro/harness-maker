...
| Objective record — required | `id` (equals the file stem, `[A-Z0-9-]+`), `title`, `hypothesis`, `scope[]` (non-empty), `outcome_id` (must exist in `intent.outcomes` at read time — see Referential breakage), `state ∈ {proposed, active, closed, dropped}`, `created_at`, `schema_version` | `needs_revalidation` and `approval_valid` are **not** fields |
...
| Storage root | `.claude/intent.yaml`, `.claude/world/{assumptions,outcomes}.yaml`, `.claude/world/objectives/<id>.yaml` | churn machinery is keyed to `.claude/` and `work-docs/` |
| Path ownership | all four patterns → `deliverable` via `_is_deliverable_path`, matching real `objectives/<id>.yaml` files; not in the churn prefix tuple; nothing gitignored | an unregistered live file classifies as `user` and finalize stashes it |
