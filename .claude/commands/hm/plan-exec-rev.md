...
harness_maker_version: 0.41.1
generated_at: '2026-01-01T00:00:00+00:00'
source_template: commands/hm/workflow_command.md.j2
provenance: official
content_hash: 301d6862fd564c9e2c884269d127a3b4d405ecf7fd9abe55124efd311a05b180
---
...
> `uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.autopilot on --level auto_safe --pipeline research,spec,plan,execute,review,verify,wrapup`
...
```bash
!uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.memory_retrieve --topic "<topic>" --k 6 --pre-k 30
```
...
```bash
!uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.second_brain search '<task slug or topic>' --type decision
!uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.second_brain search '<task slug or topic>' --type preference
!uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.second_brain search '<task slug or topic>' --type project
```
...
```bash
!uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.worktree task-preflight <slug> "$(pwd)"
```
...
```bash
!uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.worktree task-refresh <slug> "$(pwd)"
```
...
```bash
!uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.worktree loop-mode-active "<PROJECT_ROOT>" --claude-session-id "$HM_SESSION_ID"
```
...
```bash
if [ "$exit" -eq 0 ]; then uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.codex_adapter adapt --model codex < "$out_tmp"; fi; rm -f "$prompt_tmp" "$out_tmp"
```
...
```bash
reason="<one-line cause>"; uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.codex_ledger emit --slug "<slug>" --stage plan --model codex --finding-ref "n/a" --disposition unresolved --status skipped --skip-reason "$reason"
```
...
```bash
if [ "$exit" -eq 0 ]; then uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.codex_adapter adapt --model antigravity < "$out_tmp"; fi; rm -f "$prompt_tmp" "$out_tmp"
```
...
```bash
reason="<one-line cause>"; uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.codex_ledger emit --slug "<slug>" --stage plan --model antigravity --finding-ref "n/a" --disposition unresolved --status <skipped|failed> --skip-reason "$reason"
```
...
!if [ -f "<WT>/.claude/.hm-iter-receipts/.current-iter" ]; then \
   ...
   if [ -n "$ITER" ]; then \
     uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.iter_receipts write \
...
```bash
!cd <WT> && uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.spec_mutation gate --yaml specs/SPEC-{slug}.machine.yaml --tier 1
```
...
!uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.worktree finalize <WT> stage-only
```

```bash
# Stage halted on a blocker — preserve the worktree for inspection:
!uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.worktree finalize <WT> fail
```
...
```bash
!uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.worktree owned-crumb-add "$(pwd)" <slug> "$(uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.worktree wt-uuid <WT>)"
```
...
```bash
!HM_OWNED_SESSION_UUIDS="$(uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.worktree owned-crumb-read "$(pwd)" <slug>)" uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.worktree post-commit-pop "$(pwd)"
```
...
```bash
!uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.second_brain search '<changed area or task slug>' --type failure
!uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.second_brain search '<changed area or task slug>' --type preference
```
...
1. Build `pass1_context` from the diff context with PR title / description /
   ...
   ```bash
   echo '<full_context_json>' | uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.two_pass_review redact
   ```
...
4. Merge the two passes via the harness CLI:
   ...
   ```bash
   echo '{"pass1": [...], "pass2": [...]}' | uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.two_pass_review merge
   ```
...
- Side preset → invoke **every** enabled model only on a **high-diff** change. Classify first —
  ...
  ```bash
  files=$(git diff --name-only HEAD); added=$(git diff --numstat HEAD | awk '{s+=$1} END{print s+0}'); printf '%s\n' "$files" | uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.high_diff classify --added-lines "$added"
  ```
...
```bash
reason="<one-line cause>"; uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.codex_ledger emit --slug "<slug>" --stage review --model codex --finding-ref "n/a" --disposition unresolved --status skipped --skip-reason "$reason"
```
...
```bash
reason="<one-line cause>"; uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.codex_ledger emit --slug "<slug>" --stage review --model antigravity --finding-ref "n/a" --disposition unresolved --status <skipped|failed> --skip-reason "$reason"
```
...
```bash
echo '<record_json>' | uv run --with /home/noel/.claude/plugins/cache/harness-maker/harness-maker/0.41.1 python -m harness_maker.review_telemetry emit
```
