...
**Instance 12 (2026-09-12, workflow-steps-vs-model-capability) is a confirmation-pass catch, not a round-N+1 dispatch, and it caught two in one pass:** confirm-1's repair round fixed a P1 (`atomic_append` lacked a PIPE_BUF guard) by adding a short-write retry loop, and separately rewrote a verify note to claim a fail-closed gate; confirm-2 found both were themselves defects — the retry loop reopened the exact interleave race a sibling function in the same file explicitly documents as "do NOT loop" for ([[fail:design hand-rolled-when-sibling-exists]], count now 2), and the gate claim held only for one of two presets. Both survived ruff/mypy/pytest and the first confirmation pass; only a second confirmation pass (an independent re-review of the repair delta, which is exactly this proposal's mechanism) caught them. Evidence that the mechanism works when actually run twice — the gap here was that only one confirmation pass is budgeted per repair round by default.
**Instance 13 (2026-09-17, outcome-measure):** the round-2 fix for AC-003 (adding a lock + process-group kill on timeout) introduced three confirm-1 P1s in the fix itself — the lock file placed in a tracked directory, an unguarded kill fallback, and no test exercising concurrent lock acquisition. Same shape as every prior instance: the repair's own local verification (green suite) was the only evidence, and it was insufficient.
...
**Triggered by:** [fail:test] shipped-entry-point-not-exercised (count: 5 as of 2026-08-20)
...
**2026-08-20 recurrence (5th instance):** `review.md.j2:518` rendered `--spec` after `| tee <file>`, so the shell handed `--spec` to `tee` (unrecognised long option, exit 1). Every existing gate read the rendered TEXT (snapshot fixtures compare body hashes); this is the SHELL-PARSE variant of the same class — green at the template-text level, wrong at the shell the rendered line is actually handed to. The repair this time was narrower than the standing proposal: `bash -n` on the substituted line plus a structural "nothing but `tee` and its file after the pipe" check plus a `pipefail` assertion (`tests/structural/test_rendered_pipelines_parse.py`), scoped to producer pipelines rather than the general argv-seam gate proposed above. The general gate is still not built; this is a point fix on the fifth occurrence, not the mechanism.
...
## Proposal: an exit criterion that names a path must resolve to node ids at write time (2026-08-21)
**Triggered by:** [fail:test] exit-criterion-names-wrong-suite (count: 3 as of 2026-08-21; this proposal was written at count: 3) — the third instance (bench-study-adoption) was a wrapup-stage round-trip check run against `tests/structural/test_...` and `tests/unit/test_...` files that do not contain any round-trip counter at all; the real counters (`test_roundtrip_budget.py`, `test_surface_baseline.py`) live elsewhere. It read green while asserting nothing about round trips, the same shape as the first instance (PLAN-dep-map-alias-imports Phase 3) and the second (workflow-time-token-savings execute Phase D, hand-picked scope defeating the provided selector).
**Proposed mechanism:** when a PLAN, stage template, or wrapup step writes an exit/verification criterion that names a test path, require `pytest --collect-only <path> -q | head` to be run at authoring time and the resulting node count pasted next to the criterion — the same discipline `spec_machine mark-tested`/`find-unbound` already enforce for AC↔test bindings (a recorded id must COLLECT or the gate fails closed), generalized to prose-level criteria that currently have no collection check at all. A cheap first cut: a structural test that greps PLAN/REVIEW/wrapup-step criteria for a bare `pytest <path>` clause and asserts a `# collected: N` comment sits beside it.
...
**Proposed mechanism:** for `/hm:review`'s auto-fix loop, move Step 3.4's `persist-payload` call (and any future per-round obligation) out of the linear numbered procedure and into the loop body's own re-entry checklist, so round 2..N literally cannot skip it without an explicit "why" — the same fix mitigation (a) already named twice in the entry. Pair it with mitigation (b): emit the telemetry row and the payload file in the SAME call, so a partial round (row with no sibling payload) is structurally impossible instead of merely detectable after the fact by `tests/structural/test_review_payload_persisted.py`.
...
**Proposed mechanism:** when a PLAN phase claims a rendered stage's prose now invokes a Python module/CLI (a `write_rollup`, `smoke_check --targets`, `_warn_context_lint`-shaped change), require the phase's exit criterion to include a POSITIVE observation of the effect — a `caplog` record, a written output file, or a changed verdict from actually running the rendered recipe — not merely a structural grep that the call site exists in source. The entry names the exact miss: a gate asserting "a `Call` named `lint` exists in `src/`" stayed green after the only caller of that call was deleted, because the call sits inside the now-uncalled function itself.
...
## Proposal: the default retrieval byte-cap discards 90% of the pre-filtered set (2026-09-19)
**Triggered by:** measured during PLAN-observed-harness-gaps-salvage (first measured 2026-08-14 by the abandoned PLAN-observed-harness-gaps, never filed there because that task never landed), not a `[fail:*]` recurrence.
**Proposed mechanism:** re-derive the default `memory_retrieve --byte-cap` against the real corpus, or make the lexical section's cap scale with `k`/`pre_k` the way the count floor's budget now scales with `--count-floor`.
**Rationale:** re-measured 2026-09-19, topic `worktree review telemetry failure memory proposal`, `--k 6 --pre-k 30`: byte-cap 10240 → **3** entries, 40960 → 9, 200000 → 30. Every stage asks for 6 and receives 3. The count floor recovers up to 3 high-recurrence entries on top; this starvation is independent of it and changes every stage's context at once, so it needs its own budget decision (SPEC-observed-harness-gaps-salvage Non-Goal, user decision 2026-09-19).

## Proposal: wrapup's delegated path skips 5.7 (2026-09-19)
**Triggered by:** read of `templates/stages/wrapup.md.j2` during PLAN-observed-harness-gaps-salvage (R8), not a `[fail:*]` recurrence.
**Proposed mechanism:** make 5.7 reachable on the delegated path — either renumber it into the main-loop span (after Step 0.5's reconciliation) or change the delegated path's "skip straight to Step 6" to name 5.7 as the resume point; add a render test asserting the main loop is told to run 5.7 on both the delegated and the degraded path.
**Rationale:** Step 0.5 delegates "Steps 1 through 5.6" (≈:93-94), and both the reconcile branch (≈:115 "proceed to Step 6") and the inline-body preamble (≈:130 "skip straight to Step 6") send the main loop to Step 6 — but `#### 5.7 World state` sits at ≈:569, between them. Read literally, the intent-layer questions (assumption, objective close, outcome measure) never run when delegation is on. The same reasoning is why PLAN-observed-harness-gaps-salvage ADR-006 refused to put the backlog line in a new 5.8.

## Proposal: spec_gate resolves specs/ at the base repo, so it blocks every new test in a task worktree (2026-09-19)
**Triggered by:** the gate blocked `Write` of `tests/unit/test_render_wrapup_backlog_warning.py` during PLAN-observed-harness-gaps-salvage although the task's SPEC (in the worktree) references that file.
**Proposed mechanism:** resolve the SPEC directory from the target file's own checkout (walk up from `tool_input.file_path` to its git toplevel) instead of `Path.cwd()`; add a test that a test file under `.worktrees/<slug>/tests/` is allowed when `.worktrees/<slug>/specs/SPEC-*.md` references it and the base `specs/` does not.
**Rationale:** `spec_gate.main` uses `project_dir = Path.cwd()`, and hook cwd is the base repo. Under `worktree.enabled` every task's SPEC lives only in its worktree until wrapup lands it, so the gate is a false positive on the normal Production path — the operator's only moves are to write the test through a tool the gate does not watch (done here, disclosed) or to switch `dev_mode`, both of which defeat the gate.

## Proposal: route wrapup Step 5.3's proposal writes through a locked memory CLI verb (2026-09-19)
