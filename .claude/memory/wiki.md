...
ADR-006: per-session marker files replace the single global `.hm-loop-active`. Pattern: `_write_loop_marker(project_root, primary_wt_name, wt_paths)` writes `.claude/.hm-loop-{primary_wt_name}` (newline-joined absolute paths). `_clear_loop_marker(project_root, wt_name)` deletes only that file. `_detect_existing_worktree` drops the marker-based Signal 1 — each session creates its own WT, no sentinel needed. Gate upgrade (Phase 3): glob `.claude/.hm-loop-*`, union all paths, ANY-match. Key invariant: marker kept on `finalize fail` so gate continues protecting surviving worktrees; cleared only on full success. Marker filename is gitignored via `_ensure_gitignore_entry` with pattern `.claude/.hm-loop-*`.
