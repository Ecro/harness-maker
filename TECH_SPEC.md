...
### Solution
**harness-maker** — a multi-target harness generator. A **single meta-command** `/harness-maker:make` generates a *custom harness (commands, skills, agents, hooks, monitoring, anti-rot, worktree, and security assets)* via an interview-driven flow. Claude Code uses `.claude/`; Cursor reuses the same `.claude/` assets and receives `.cursor/` glue; Codex receives `AGENTS.md`, `.codex/`, and `.agents/skills/`. Day-to-day commands use the `/hm:` prefix where slash commands are available, and matching skills where Codex discovers skills. Supports both Brownfield and Greenfield-with-spec projects. Obsidian Second Brain support connects a generated harness to a user-owned Markdown vault as typed stage-aware memory; writable folders require a `project_id` namespace so multiple projects sharing one vault do not collide.
