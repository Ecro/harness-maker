...
> **Codex CLI install path.** 최근 Codex CLI (0.144.4 에서 확인) 는 `codex plugin marketplace add` + `codex plugin add harness-maker@harness-maker` 로 이 플러그인을 실제로 설치합니다 — 다만 repo 를 plugin cache 에 clone 할 뿐, 하네스를 렌더하는 Python 엔진은 설치하지 않습니다. 따라서 신뢰 가능한 경로는 여전히 Claude Code marketplace (`claude` CLI 가 PATH 에 있을 때) 또는 PyPI (`uv tool install harness-maker`) 로 엔진을 설치하는 것이며, 두 경로 모두 디스크에 동일한 `.codex/` 산출물을 생성합니다. native `codex plugin add` 는 그 위에 얹는 선택사항이지 엔진 설치의 대체가 아닙니다.
...
#### Codex CLI (native plugin 설치는 되지만, 엔진은 Claude Code 또는 PyPI 에서)

**최근 Codex CLI (0.144.4 에서 확인) 는 이 플러그인을 native 로 설치합니다** — `codex plugin marketplace add Ecro/harness-maker` 후 `codex plugin add harness-maker@harness-maker` 가 성공하며, repo 를 `~/.codex/plugins/cache/` 로 clone 합니다. **다만 그 clone 은 하네스를 실제로 렌더하는 Python 엔진 (`harness-maker` / `python -m harness_maker`) 을 설치하지 않으며**, clean release 가 아니라 working tree 전체 (`.git`, 캐시, 백업) 를 끌고 옵니다. 따라서 native `plugin add` 하나에 의존하지 말고 — 아래 두 경로 중 하나로 엔진을 설치하세요. native `codex plugin add` 는 그 위에 얹는 선택사항이지 대체가 아닙니다. `.codex-plugin/plugin.json` manifest 는 metadata 만 담습니다 (install hook 없음).
