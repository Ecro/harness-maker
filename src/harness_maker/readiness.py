...
def _dim_guardrails(project_dir: Path, *, session_id: str | None = None) -> DimensionScore:
    ...
    # Render-drift guard (PLAN-wrapup-waiver-enforcement ADR-004/C5): the oracle-waiver
    # advisory (wrapup Step 3.6) is baked at render time and renders only at `warn`
    # strictness. If harness.yaml's strictness changed without re-rendering, the rendered
    # wrapup either silently LACKS the advisory (warn) or mis-fires it (block). N-A (no
    # signal) when the wrapup command or a readable harness.yaml is absent.
    wrapup_cmd = claude / "commands" / "hm" / "wrapup.md"
    _strictness: str | None = None
    ...
    if _hy_path.is_file():
        try:
            ...
            _hy2 = _lhy(_hy_path)
            _strictness = resolve_strictness(_hy2) if isinstance(_hy2, dict) and _hy2 else None
        except Exception:  # noqa: BLE001 — degrade to N-A, never crash readiness
            _strictness = None
    _has_advisory: bool | None = None
    if wrapup_cmd.is_file() and _strictness is not None:
        try:
    ...
    if _has_advisory is not None and _strictness is not None:
        _matched = _has_advisory == (_strictness == "warn")
        signals.append(
            _signal(
                "wrapup_oracle_waiver_strictness_match",
                _matched,
                15,
                "rendered wrapup oracle-waiver advisory matches harness.yaml strictness"
                if _matched
                else f"rendered wrapup advisory MISMATCHES strictness={_strictness} "
                ...
                "matches the current strictness (oracle-waiver advisory render-drift)",
            )
        )
